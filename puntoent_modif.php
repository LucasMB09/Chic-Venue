...
<body>
  ...
  <section class="formulario">
    <center><h3 class="h1ME">Modificar Datos de la Dirección: <?php echo $_POST['nom_punto'];?></h3></center>
    <div class="contenidoform">
      <p>Porfavor ayudanos llenando los siguientes campos</p>
      <form action="/php/editar_punto_entrega.php" method="post" enctype="multipart/form-data" id="form_punto_ent">

        <!--Nombre del punto de entrega-->
        <label for="nombre_punto">Nombre del punto de entrega*:</label>
        <input type="text" id="nombre_punto" name="nombre_punto" value="<?php echo $_SESSION['nom_pun'] = $_POST['nom_punto'];?>" required>
        <div id="NombrepuntErrormessage" class="error-message"></div>
        <div class="error"></div>

        <!--Nombre destinatario-->
        <label for="nombre_destinatario">Nombre del destinatario*:</label>
        <input type="text" id="nombre_destinatario" name="nombre_destinatario" value="<?php echo $_POST['nom_dest'];?>"required>
        ...
        <input type="text" id="calle" name="calle" placeholder="Calle" value="<?php echo $_POST['calle'];?>" required>
        <!--Calle-->
        <div id="CalleErrormessage" class="error-message"></div>
        <div class="error"></div>
        <!--Numero-->
        <input type="text" id="numero" name="numero" placeholder="Número" value="<?php echo $_POST['numero'];?>" required>
        <div id="NumdirErrormessage" class="error-message"></div>
        <div class="error"></div>
        <!--Colonia-->
        <input type="text" id="colonia" name="colonia" placeholder="Colonia" value="<?php echo $_POST['colonia'];?>"required>
        <div id="ColoniaErrormessage" class="error-message"></div>
        <div class="error"></div>
        <!--Ciudad-->
        <input type="text" id="ciudad" name="ciudad" placeholder="Ciudad" value="<?php echo $_POST['ciudad'];?>" required>
        <div id="CiudadErrormessage" class="error-message"></div>
        <div class="error"></div>
        <!--Estado-->
        <select id="estado" name="estado">
          <option value="Estado/Provincia">Estado/Provincia</option>
          <option value="Aguascalientes" <?php if ($_POST['estado'] == 'Aguascalientes') echo 'selected'; ?>>Aguascalientes</option>
          <option value="Baja California" <?php if ($_POST['estado'] == 'Baja California') echo 'selected'; ?>>Baja California</option>
          <option value="Baja California Sur" <?php if ($_POST['estado'] == 'Baja California Sur') echo 'selected'; ?>>Baja California Sur</option>
          <option value="Campeche" <?php if ($_POST['estado'] == 'Campeche') echo 'selected'; ?>>Campeche</option>
          <option value="Chiapas" <?php if ($_POST['estado'] == 'Chiapas') echo 'selected'; ?>>Chiapas</option>
          <option value="Chihuahua" <?php if ($_POST['estado'] == 'Chihuahua') echo 'selected'; ?>>Chihuahua</option>
          <option value="Ciudad de México" <?php if ($_POST['estado'] == 'Ciudad de México') echo 'selected'; ?>>Ciudad de México</option>
          <option value="Coahuila" <?php if ($_POST['estado'] == 'Coahuila') echo 'selected'; ?>>Coahuila</option>
          <option value="Colima" <?php if ($_POST['estado'] == 'Colima') echo 'selected'; ?>>Colima</option>
          <option value="Durango" <?php if ($_POST['estado'] == 'Durango') echo 'selected'; ?>>Durango</option>
          <option value="Estado de México" <?php if ($_POST['estado'] == 'Estado de México') echo 'selected'; ?>>Estado de México</option>
          <option value="Guanajuato" <?php if ($_POST['estado'] == 'Guanajuato') echo 'selected'; ?>>Guanajuato</option>
          <option value="Guerrero" <?php if ($_POST['estado'] == 'Guerrero') echo 'selected'; ?>>Guerrero</option>
          <option value="Hidalgo" <?php if ($_POST['estado'] == 'Hidalgo') echo 'selected'; ?>>Hidalgo</option>
          <option value="Jalisco" <?php if ($_POST['estado'] == 'Jalisco') echo 'selected'; ?>>Jalisco</option>
          <option value="Michoacán" <?php if ($_POST['estado'] == 'Michoacán') echo 'selected'; ?>>Michoacán</option>
          <option value="Morelos" <?php if ($_POST['estado'] == 'Morelos') echo 'selected'; ?>>Morelos</option>
          <option value="Nayarit" <?php if ($_POST['estado'] == 'Nayarit') echo 'selected'; ?>>Nayarit</option>
          <option value="Nuevo León" <?php if ($_POST['estado'] == 'Nuevo León') echo 'selected'; ?>>Nuevo León</option>
          <option value="Oaxaca" <?php if ($_POST['estado'] == 'Oaxaca') echo 'selected'; ?>>Oaxaca</option>
          <option value="Puebla" <?php if ($_POST['estado'] == 'Puebla') echo 'selected'; ?>>Puebla</option>
          <option value="Querétaro" <?php if ($_POST['estado'] == 'Querétaro') echo 'selected'; ?>>Querétaro</option>
          <option value="Quintana Roo" <?php if ($_POST['estado'] == 'Quintana Roo') echo 'selected'; ?>>Quintana Roo</option>
          <option value="San Luis Potosí" <?php if ($_POST['estado'] == 'San Luis Potosí') echo 'selected'; ?>>San Luis Potosí</option>
          <option value="Sinaloa" <?php if ($_POST['estado'] == 'Sinaloa') echo 'selected'; ?>>Sinaloa</option>
          <option value="Sonora" <?php if ($_POST['estado'] == 'Sonora') echo 'selected'; ?>>Sonora</option>
          <option value="Tabasco" <?php if ($_POST['estado'] == 'Tabasco') echo 'selected'; ?>>Tabasco</option>
          <option value="Tamaulipas" <?php if ($_POST['estado'] == 'Tamaulipas') echo 'selected'; ?>>Tamaulipas</option>
          <option value="Tlaxcala" <?php if ($_POST['estado'] == 'Tlaxcala') echo 'selected'; ?>>Tlaxcala</option>
          <option value="Veracruz" <?php if ($_POST['estado'] == 'Veracruz') echo 'selected'; ?>>Veracruz</option>
          <option value="Yucatán" <?php if ($_POST['estado'] == 'Yucatán') echo 'selected'; ?>>Yucatán</option>
          <option value="Zacatecas" <?php if ($_POST['estado'] == 'Zacatecas') echo 'selected'; ?>>Zacatecas</option>
        </select>
        <div id="EstadoErrormessage" class="error-message"></div>
        <div class="error"></div>
        <!--Codigo Postal-->
        <input type="text" id="codigo_postal" name="codigo_postal" placeholder="Código Postal" value="<?php echo $_POST['cp'];?>" required>
        <div id="CPErrormessage" class="error-message"></div>
        <div class="error"></div>

        <!--Informacion adicional-->
        <label for="informacion_adicional">Información adicional:</label>
        <textarea id="informacion_adicional" name="informacion_adicional"><?php echo $_POST['info_adi'];?></textarea>

        <!--Contacto de destinatario-->
        <label for="contacto">Contacto del punto de entrega:</label>
        <!--Numero de telefono-->
        <input type="tel" id="telefono" name="telefono" placeholder="Telefono" value="<?php if(isset($_POST['telefono'])){ echo $_POST['telefono'];}?>" required maxlength="10">
        <div id="TelefonoErrormessage" class="error-message"></div>
        <div class="error"></div>
        <!--Email-->
        <input type="email" id="email" name="email" placeholder="  Correo Electrónico" value="<?php if(isset($_POST['correo'])){ echo $_POST['correo'];}?>" required>
        <div id="MailErrormessage" class="error-message"></div>
        <div class="error"></div>

        <div>
          <button type="submit" class="boton_avanzar" id="agregarBtn">Modificar punto de entrega</button>
        </div>
      </form>
    </div>
  </section>
